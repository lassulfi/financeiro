...
public class YahooFinanceUtil {
	...
	public static final String SIGA_BOVESPA = "^BVSP";
	...
	public static final int INDICE_VOLUME_NEGOCIADO_DIA_ACA0 = 8;
	...
	public static String getInfoCotacao(int indiceInformacao, Acao acao) throws UtilException {
		...
		try {
			...
			String linha = null;
			String[] informacaoesCotacao = null;
			if ((linha = arquivo.readLine()) != null) {
				arquivo.close();
				conteudo.close();
				linha = linha.replace("\"", "");
				informacaoesCotacao = linha.split("[;,]");
				String valor = informacaoesCotacao[indiceInformacao];
				if (!"N/A".equals(valor)) {
					return valor;
				}
			}
